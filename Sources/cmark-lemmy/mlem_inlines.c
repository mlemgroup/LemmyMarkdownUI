...
cmark_node_type CMARK_NODE_STRIKETHROUGH, CMARK_NODE_SUBSCRIPT, CMARK_NODE_SUPERSCRIPT;
...
static cmark_node *match(cmark_syntax_extension *self, cmark_parser *parser,
  ...
  if ((left_flanking || right_flanking) && (delims == 1) || (delims == 2 && character == '~')) {
...
static delimiter *insert(cmark_syntax_extension *self, cmark_parser *parser,
  ...
  if (opener->delim_char == '^') {
      node_type = CMARK_NODE_SUPERSCRIPT;
  } else {
      node_type = opener->inl_text->as.literal.len == 1 ? CMARK_NODE_SUBSCRIPT : CMARK_NODE_STRIKETHROUGH;
  }
  if (!cmark_node_set_type(node, node_type))
    goto done;
...
static int can_contain(cmark_syntax_extension *extension, cmark_node *node,
  ...
  if (!(node->type == CMARK_NODE_STRIKETHROUGH || node->type == CMARK_NODE_SUBSCRIPT || node->type == CMARK_NODE_SUPERSCRIPT))
    return false;
...
cmark_syntax_extension *create_mlem_inlines_extension(void) {
  cmark_syntax_extension *ext = cmark_syntax_extension_new("mlem_inlines");
